...
func main() {
	...
	mux.Handle("POST /media-entries/{id}/comments", auth.RequireAuth(http.HandlerFunc(h.CreateComment)))
	mux.HandleFunc("GET /media-entries/{id}/comments", h.ListMediaEntryComments)
	mux.Handle("DELETE /comments/{id}", auth.RequireAuth(http.HandlerFunc(h.DeleteComment)))
